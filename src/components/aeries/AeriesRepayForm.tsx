...
import { repayAriesToken } from "../../actions/aeries/repay";
import { formatAddress, getExplorerUrl } from "../../utils/aptos";

interface AeriesRepayFormProps {
  privateKey: string;
  address: string;
  onOperationComplete?: () => void;
}

export default function AeriesRepayForm({
  privateKey,
  address,
  onOperationComplete,
}: AeriesRepayFormProps) {
  ...
  async function handleSubmit(values: { assetType: string; amount: string; decimals: string }) {
    ...
    const confirmMessage = `Repay ${amount} tokens (${values.assetType.slice(0, 20)}...) from ${formatAddress(address)}?`;
    ...
    try {
      const hash = await repayAriesToken(privateKey, values.assetType, amount, decimals, "mainnet");
  ...
  return (
    ...
    >
      <Form.Description
        title="Repay Borrowed Tokens to Aeries"

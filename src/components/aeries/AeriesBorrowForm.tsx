...
import { borrowAriesToken } from "../../actions/aeries/borrow";
import { formatAddress, getExplorerUrl } from "../../utils/aptos";

interface AeriesBorrowFormProps {
  privateKey: string;
  address: string;
  onOperationComplete?: () => void;
}

export default function AeriesBorrowForm({
  privateKey,
  address,
  onOperationComplete,
}: AeriesBorrowFormProps) {
  ...
  async function handleSubmit(values: { assetType: string; amount: string; decimals: string }) {
    ...
    const confirmMessage = `Borrow ${amount} tokens (${values.assetType.slice(0, 20)}...) to ${formatAddress(address)}?`;
    ...
    try {
      const hash = await borrowAriesToken(privateKey, values.assetType, amount, decimals, "mainnet");
  ...
  return (
    ...
    >
      <Form.Description
        title="Borrow Tokens from Aeries"
